...
var CartDrawer = class extends Drawer {
  ...
  async _onCartChanged(event) {
    ...
    if (event.detail.cart["item_count"] > 0) {
      ...
      /* Clear cart Attribure if cart is empty */        
        if (Object.keys(event.detail.cart["attributes"]).length > 0) {
          var nullHash = {};
          var obj_attributes = event.detail.cart["attributes"];
          for (const [key, value] of Object.entries(obj_attributes)) {
            nullHash[key] = null;
          }          
          fetch('/cart/update.js', {
            ...
            if (response.ok) {
              console.log('Cart attributes cleared');
            } else {
              console.log('Failed to clear cart attributes');
            }
          });
        }
      /* Clear cart Attribure if cart is empty */
